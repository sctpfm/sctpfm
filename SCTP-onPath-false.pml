...
chan AtoC = [0] of { mtype:msgs, mtype:tag, mtype:tag }; /* chunk, vtag, itag */
chan CtoA = [0] of { mtype:msgs, mtype:tag, mtype:tag }; /* chunk, vtag, itag */
chan BtoC = [0] of { mtype:msgs, mtype:tag, mtype:tag }; /* chunk, vtag, itag */
chan CtoB = [0] of { mtype:msgs, mtype:tag, mtype:tag }; /* chunk, vtag, itag */
...
init {
   ...
   run sctp(CtoA, AtoC, UserA);
   run sctp(CtoB, BtoC, UserB);
   run user(UserA);
   run user(UserB);
}
...
COOKIE_WAIT:
   ...
   :: in ? INIT,NO_TAG,EXP_TAG -> out ! INIT_ACK,EXP_TAG,EXP_TAG;
   :: in ? INIT,NO_TAG,UNEX_TAG -> out ! INIT_ACK,UNEX_TAG,EXP_TAG;
   :: full(in)                             &&
      (!(in ? [INIT,NO_TAG,_]))            &&
      (!(in ? [INIT_ACK,EXP_TAG,EXP_TAG])) &&
