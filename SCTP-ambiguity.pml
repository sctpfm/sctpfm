...
COOKIE_ECHOED:
   ...
   do 
   // START OF AMBIGUITY
   :: in ? INIT,0,0 -> out ! INIT_ACK,1,1; didambg[_pid % 2] = 1;
   // END OF AMBIGUITY
   :: in ? COOKIE_ACK,1,1 ->
...
ESTABLISHED:
   ...
   :: in ? ABORT,1,1 -> handle_abort_msg(in, _pid % 2);
   // Logic for handling unexpected INIT, which was not included in default model
   :: in ? INIT,1,1 -> out ! ABORT,1,1; delete_tcb(_pid % 2); goto CLOSED;
   //
   :: full(in)                 &&
      (!(in ? [ABORT,0,1]))    &&
      (!(in ? [ABORT,1,1]))    &&
...
active proctype reorder_A() {
   AtoB ? COOKIE_ECHO,1,1;
   AtoB ! COOKIE_ECHO,1,1;
}

ltl witness_attack {
   always ((didambg[0] != 1 || state[1] != ClosedState) &&
           (didambg[1] != 1 || state[0] != ClosedState))
}